...
# s={r'/':r'...',r'444':r'555'}
#    print multiple_replace1(r'aa/b444bb/cc444c',s)
...
def randomstr(strlen):
    s= string.join(random.sample(['a','b','c','d','e','f','g','h','i','j'], int(strlen))).replace(" ","")
    return s
def makefilename(file_exe,filename_len):
    s=string.join(random.sample(['a','b','c','d','e','f','g','h','i','j'], int(filename_len))).replace(" ","")
    s=s+file_exe
def getip_str(lines):
    ls=[]
    reip = re.compile(r'(?<![\.\d])(?:\d{1,3}\.){3}\d{1,3}(?![\.\d])')
    for ip in reip.findall(lines):
        ls.append(ip)
    return ls
def coroutine(func):
    '''
...
def run(args, stdin=None, shell=False):
    ...
    if p.returncode != 0:
        e = Exception(
            'Error running command.  args=' + str(args) + ' returncode=' + str(p.returncode) + '\nstdin=' + str(
                stdin) + '\nstderr=' + str(error))
